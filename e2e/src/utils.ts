...
export const immichCli = (args: string[]) =>
  executeCommand({ command: 'node', args: ['node_modules/.bin/immich', '-d', `/${tempDir}/immich/`, ...args] });
export const immichAdmin = (args: string[]) =>
  executeCommand({
    command: 'docker',
    args: ['exec', '-i', 'immich-e2e-server', '/bin/bash', '-c', `immich-admin ${args.join(' ')}`],
  });

const executeCommand = ({ command, args }: { command: string; args: string[] }) => {
